...
from MLP2 import HiddenLayer, MLP
from logisticRegression3 import LogisticRegression 
from rbm_har2 import  RBM,GRBM
...
def read_data_set(dataset1, dataset2, one_hot = False, reshape = True):
    ...
    test = Dataset(test_x, test_y, reshape = reshape)
    return train,test
...
class DBN(object):
    ...
    def __init__(self, n_inp = 784, n_out = 10, hidden_layer_sizes = [500, 500]):
        ...
        self.x = tf.compat.v1.placeholder(tf.float32, [None, height, weight, channel])
        self.y = tf.compat.v1.placeholder(tf.float32, [None, n_out])
    ...
    def pretraining(self, sess,train_set, test_set, batch_size = 100, pretraining_epochs = 10,
        ...
        batch_num = int(train_set.num_examples / batch_size)
        ...
        for i in range(self.n_layers):
            ...
            for epoch in range(pretraining_epochs):
                ...
                for j in range(batch_num):
                    ...
                    #test = sess.run(param, feed_dict = {self.x: batch_xs, })
                    #moretest = sess.run(gparam, feed_dict = {self.x: batch_xs, })
    ...
    def fine_tuning(self, sess, train_set,test_set, batch_size = 10 , training_epochs = 1000 , learning_rate = 0.1, display_step = 1):
        ...
        for epoch in range(training_epochs):
            ...
            for i in range(batch_num):
                b =[]
                d = []
                batch_xs, batch_ys = train_set.next_batch(batch_num,batch_size)
            ...
            if epoch % display_step == 0:
                ...
                acc = sess.run(accuracy, feed_dict = {self.x: test_set.segments, self.y: test_set.labels})
                pr= sess.run(self.pred, feed_dict = {self.x: test_set.segments, self.y: test_set.labels})
                #------
                b = np.append(b, sess.run(tf.argmax(pr, axis =1)))
                #np.savetxt('b.txt', b ,delimiter=',')
                d = np.append(d , sess.run(tf.argmax(self.y, axis =1 ), feed_dict ={self.x: test_set.segments, self.y: test_set.labels}))
...
if __name__ == "__main__":
    ...
    filename1 = "datasets/kdd/kddcup.data_10_percent_corrected"
    #filename1 = dir_path + "/datasets/kddcup.data.corrected"
    filename2 = "datasets/kdd/corrected"
    ...
    with tf.compat.v1.Session() as sess:
        sess.run(init)
        dbn.pretraining(sess, train_set = train, test_set = test, k =1)
        dbn.fine_tuning(sess,trains_set = train, test_set = test)
